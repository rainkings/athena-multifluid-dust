...
Real dfloor, dffloor, pfloor;
  //snowline
Real f_ICE_inter0, m_p0, taus0, rho_sil_inter, rho_ice_inter;
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  m_p0 =  pin->GetOrAddReal("problem", "m_p0", 0.0); // initial constant pebbel mass [cgs]
  ...
  if(true){
    ...
    // ruser_mesh_data[9].NewAthenaArray(mesh_size.nx3, mesh_size.nx2+NGHOST, mesh_size.nx1 + 2*NGHOST);
  }
...
#pragma omp simd
      for (int i=istart; i<=iend; ++i) {
        // initialize tem_active
        // LogicalLocation &loc = loc;
...
void Mesh::UserWorkInLoop() {
        ...
        for (int k=pmb->ks; k<=pmb->ke; k++) {
          for (int j=pmb->js; j<=pmb->je; j++) {
            for (int i=istart; i<=iend; i++) {            
              ...
              Real fv = pmb->pdustfluids->df_w(4*(NDUSTFLUIDS-1),k,j,i) / rho_gas;
          ...
          for (int k=pmb->ks; k<=pmb->ke; k++) {
            for (int j=pmb->js; j<=pmb->je; j++) {
              for (int i=istart; i<=iend; i++) {
                ...
                // manually alter the heating profile
                // Real width_z = 0.02 * CONST_au / UNIT_LENGTH;
                // Real f_z_vis = tanh(SQR((z-width_z)/width_z)) * ((z > width_z) ? 1.0:0.0) * 2.2;
                // q_vis *= f_z_vis;

                // Real UNIT_SB = UNIT_DENSITY * UNIT_VELOCITY * SQR(UNIT_VELOCITY); // flx/(K^4)
                // Real q_bg = 0.0*CONST_sigma/UNIT_SB* 4.0*rhokappa(tk,tj,ti); // background radiation
                ////////////////////
                ...
                q_z(tk,tj,ti) = q_irr + q_vis + pmb->pphase_change->q_latent(k,j,i);
          ...
          for (int k=pmb->ks; k<=pmb->ke; k++) {
            for (int j=pmb->js; j<=pmb->je; j++) {
              for (int i=istart; i<=iend; i++) {
                ...
                // Tem = (0.75*tau_eff(tk,tj,ti) + std::sqrt(3.0)/4.0)*F_inf +  q_z(tk,tj,ti)/(4.0*rhokappa(tk,tj,ti));
                // Tem = std::pow(Tem/(CONST_sigma/UNIT_FLX) , 0.25);
      ...
      for (int k=pmb->ks; k<=pmb->ke; k++) {
        for (int j=pmb->js; j<=pmb->je; j++) {
          for (int i=istart; i<=iend; i++) {
            ...
            Real fv = pmb->pdustfluids->df_w(4*(NDUSTFLUIDS-1),k,j,i) / pmb->phydro->w(IDN,k,j,i);
            Real mu = Get_mu(fv);
        ...
        for (int j=pmb->js; j<=pmb->je; j++) {
          for (int i=istart; i<=iend; i++) {
            ...
            if(time > t_iterate and pmb->pbval->block_bcs[BoundaryFace::inner_x1] == BoundaryFlag::user and x1 < x1min){
              ...
              rho_z(tk,tj,ti) = (pmb->phydro->w(IDN,k,j,pmb->is) -pmb->pdustfluids->df_w(4*(NDUSTFLUIDS-1),k,j,pmb->is)) * SQR(x1_active)/SQR(x1_ghost);
    ...
    if (pmb->pbval->block_bcs[BoundaryFace::outer_x1] == BoundaryFlag::user){
      ...
      for (int k=pmb->ks; k<=pmb->ke; k++) {
        for (int j=pmb->js; j<=pmb->je; j++) {
          ...
          Real x1_active = pmb->pcoord->x1v(pmb->ie);
          // Real ds_active = 2.0*PI*SQR(x1_active)*std::sin(x2)*dx2; // propto
  ...
  for (int p=0; p<N_P; p++) {
    for (int n=0; n<N_Z; n++) {
      int dust_id = N_Z * p + n;
      if (Mdot_peb_est(dust_id) > 0.0) {
        flx_peb_ratio(dust_id) = Mdot_peb / Mdot_peb_est(dust_id);
      } else {
        flx_peb_ratio(dust_id) = 1.0; // Avoid division by zero (shouldn't happen if dust_rho > dffloor)
      }
    }
  }
...
void MeshBlock::UserWorkInLoop(){
  ...
  if (Damping_Flag) {
    ...
    // if(NDUSTFLUIDS > 0 and time > dust_start_injection){
    //   OuterWaveDampingDust(this, time, dt, il, iu, jl, ju, kl, ku,pdustfluids->df_w, pdustfluids->df_u);
    // }
  }
    ...
    for (int j=jl; j<=ju; ++j) {
      for (int i=il; i<=iu; ++i) {
        ...
        if(i < is or i > ie or j < js or j > je){
          ...
          Real fv = pdustfluids->df_w(4*(NDUSTFLUIDS-1), k, j, i)/gas_den;
          Real T_from_erg;
          if (pphase_change != nullptr) {
            ...
            T_from_erg = e*KELVIN/(fx/mu_H2*0.71*2.5 + fx/mu_He*0.29*1.5 + fv/mu_z*3.0);
          }
          
          phydro->Tem(k, j, i) = T_from_erg;          
        }

        // Stopping time and diffusivity calculation moved to MyStoppingTime and MyDustDiffusivity (Yu, 2025-11-18)
      }
    }
  }
}
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      for (int i=il; i<=iu; ++i) {
        ...
        user_out_var(8,k,j,i) = pdustfluids->df_flux[X1DIR](4*(NDUSTFLUIDS-1),k,j,i);
        user_out_var(9,k,j,i) = pdustfluids->df_flux[X2DIR](4*(NDUSTFLUIDS-1),k,j,i);
        // user_out_var(11,k,j,i) = phydro->hdif.nu(HydroDiffusion::DiffProcess::alpha, k, j, i);
        user_out_var(11,k,j,i) = phydro->flux[X1DIR](IDN,k,j,i);
        user_out_var(12,k,j,i) = phydro->flux[X2DIR](IDN,k,j,i);
        user_out_var(13,k,j,i) = pdustfluids->nu_dustfluids_array(NDUSTFLUIDS-1,k,j,i);
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      for (int i=il; i<=iu; ++i) {
        ...
        // user_out_var(0,k,j,i) = pmy_mesh->ruser_mesh_data[4](tk, tj, ti);
        user_out_var(2,k,j,i) = pmy_mesh->ruser_mesh_data[1](tk, tj, ti);
        user_out_var(5,k,j,i) = pmy_mesh->ruser_mesh_data[2](tk, tj, ti);
        // user_out_var(6,k,j,i) = pmy_mesh->ruser_mesh_data[0](tk, tj, ti);
        // user_out_var(7,k,j,i) = pmy_mesh->ruser_mesh_data[2](tk, tj, ti);
      }
    }
  }
...
#pragma omp simd
        for (int i=pmb->is; i<=pmb->ie; ++i) {
          ...
          Real &rho_v = cons_df(4*(NDUSTFLUIDS-1), k, j, i);
          ...
          if(time < t_iterate){
            ...
            //   // compute initial conditions in cylindrical coordinates
            //   Real den_dust = initial_D2G[dust_id]*gas_dens;
            //   pmb->pdustfluids->df_u(rho_id, k, j, i) = den_dust;
					  // }
          }
          ///////////////////////////////////////////////////////////////////
          ...
          // gas_dens = (gas_dens > dfloor +dffloor) ? gas_dens : (dfloor+dffloor);
          // gas_dens = (gas_dens > rho_v) ? gas_dens : (gas_dens + rho_v);
          Real mu = Get_mu(fv);
...
#pragma omp simd
      for (int i=is; i<=ie; ++i) {
        ...
        Real fv = pmb->pdustfluids->df_w(4*(NDUSTFLUIDS-1), k, j, i) / gas_rho;
        Real mu = Get_mu(fv);
        Real kappa_R = Get_kappa(0.0, fv);
        Real UNIT_SB = pmb->pmy_mesh->punit->code_density_cgs * pmb->pmy_mesh->punit->code_velocity_cgs * SQR(pmb->pmy_mesh->punit->code_velocity_cgs); // flx/(K^4)
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      ...
      for (int i=1; i<=ngh; ++i) {
        ...
        Real fv = prim_df(4*(NDUSTFLUIDS-1), k, j, il)/gas_rho_active;
        
        int tk = 0; int ti = il-i;
        int tj = static_cast<int>(pmb->loc.lx2)*pmb->block_size.nx2+(j-pmb->js)+ NGHOST;
        int tj_mid = pmb->pmy_mesh->mesh_size.nx2 + NGHOST -1;
        Real Tem_mid = pmb->pmy_mesh->ruser_mesh_data[4](tk,tj_mid,ti);
        // Real Tem_ghost = pmb->pmy_mesh->ruser_mesh_data[4](tk,tj,ti);
        
        // rotational vel
        // Real qnew = five_point(pmb->pmy_mesh->ruser_mesh_data[4], rad_active, pco->dx1v(il), tk, tj_mid, NGHOST);
        // Real ps = pvalue + 1.5 + qvalue/2.0;
        // Real pnew = ps - (qnew/2.0 + 1.5);
        // Real vel_gas_phi = VelProfileCyl_gas_fv_T_pvalue(rad_ghost, phi_ghost, z_ghost, fv, Tem_ghost, pnew, qnew);
        // if (pmb->porb->orbital_advection_defined)
        //   vel_gas_phi -= vel_K;
        ...
        // // free outflow bc for damping zone
        if(x2 <= theta_upper_damping and Theta_Gas_Damping_Flag){
          // gas_rho_ghost = gas_rho_active*SQR(x1_active)/ SQR(x1_ghost);
          gas_rho_ghost = std::exp(2.0*std::log(prim(IDN, k, j, il-i+1)) - std::log(prim(IDN, k, j, il-i+2)));
          Real mu1 = Get_mu(fv);
          gas_pres_ghost = gas_rho_ghost*Tem_ghost/(mu1*KELVIN);
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      ...
      for (int i=1; i<=ngh; ++i) {
        ...
        // rotational vel
        // Real qnew = five_point(pmb->pmy_mesh->ruser_mesh_data[4], rad_active, pco->dx1v(iu), tk, tj_mid, pmb->pmy_mesh->mesh_size.nx1 + NGHOST-1);
        // Real ps = pvalue + 1.5 + qvalue/2.0;
        // Real pnew = ps - (qnew/2.0 + 1.5);
        ...
        // Real vel_gas_phi = VelProfileCyl_gas_fv_T_pvalue(rad_ghost, phi_ghost, z_ghost, 0.0, Tem_mid, pnew, qnew);
        // Real vr0 = Vr_ProfileCyl_gas_fv_T_pvalue(rad_active, phi_active, z_active, 0.0,Tem_mid, pnew, qnew);
        // Real vel_gas_phi = VelProfileCyl_gas(rad_ghost, phi_ghost, z_ghost);
        // Real vr0 = Vr_ProfileCyl_gas(rad_ghost, phi_ghost, z_ghost);
        ...
        if (NON_BAROTROPIC_EOS)
          ...
          // gas_pres_ghost = NewtonIntpl(prim(IPR, k, j, iu+i-3),prim(IPR, k, j, iu+i-2),prim(IPR, k, j, iu+i-1),pco->x1v(iu+i-3),pco->x1v(iu+i-2),pco->x1v(iu+i-1),x1_ghost);
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=1; j<=ngh; ++j) {
      for (int i=il; i<=iu; ++i) {
        ...
        Real fv = prim_df(4*(NDUSTFLUIDS-1), k, jl, i)/gas_rho_active;
...
#pragma omp simd
      for (int i=il; i<=iu; ++i) {
        ...
        if (x2 <= theta_upper_damping and x2 > x2min) {
          ...
          // if(rad_arr(i) <= radius_inner_damping and rad_arr(i) > rad_min){
          //   Real R_func      = SQR((rad_arr(i) - radius_inner_damping)*inv_inner_damp) * 2.0;
          //   R_func           = R_func > 1.0 ? 1.0 : R_func;
          //   Theta_func(i)    *= (1.0-R_func);
          // }
          // if(rad_arr(i) >= radius_outer_damping and rad_arr(i) < rad_max){
          //   Real R_func      = SQR((rad_arr(i) - radius_outer_damping)*inv_outer_damp) * 2.0;
          //   R_func           = R_func > 1.0 ? 1.0 : R_func;
          //   Theta_func(i)    *= (1.0-R_func);
          // }
          ...
          ////////////////////////////////////////////
          // damp to initial condition
          // Real fv = pmb->pdustfluids->df_w(4*(NDUSTFLUIDS-1),k,j,i) / prim(IDN, k, j, i);
          // Real gas_rho_0   = DenProfileCyl_gas(rad_arr(i), phi_arr(i), z_arr(i));
          // Real cs_square_0 = PoverRho(rad_arr(i), phi_arr(i), z_arr(i));
          
          // Real vis_vel_cyl  = Vr_ProfileCyl_gas(rad_arr(i), phi_arr(i), z_arr(i));
          // Real vel_gas_phi  = VelProfileCyl_gas(rad_arr(i), phi_arr(i), z_arr(i));
          // Real vis_vel_cyl = Vr_ProfileCyl_gas_fv(rad_arr(i), phi_arr(i), z_arr(i), fv);
          // Real vel_gas_phi = VelProfileCyl_gas_fv(rad_arr(i), phi_arr(i), z_arr(i), fv);
