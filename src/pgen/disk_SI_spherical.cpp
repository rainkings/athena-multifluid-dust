...
Real DustFluidsRatioMaximum_1(MeshBlock *pmb, int iout);
Real DustFluidsRatioMaximum_2(MeshBlock *pmb, int iout);
Real DustFluidsRatioMaximum_3(MeshBlock *pmb, int iout);
Real DustFluidsRatioMaximum_4(MeshBlock *pmb, int iout);
Real DustFluidsRatioMaximum_5(MeshBlock *pmb, int iout);
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  if (NDUSTFLUIDS > 0) {
    ...
    EnrollUserHistoryOutput(0, DustFluidsRatioMaximum_1, "RatioMax-1", UserHistoryOperation::max);
    EnrollUserHistoryOutput(1, DustFluidsRatioMaximum_2, "RatioMax-2", UserHistoryOperation::max);
    EnrollUserHistoryOutput(2, DustFluidsRatioMaximum_3, "RatioMax-3", UserHistoryOperation::max);
    EnrollUserHistoryOutput(3, DustFluidsRatioMaximum_4, "RatioMax-4", UserHistoryOperation::max);
    EnrollUserHistoryOutput(4, DustFluidsRatioMaximum_5, "RatioMax-5", UserHistoryOperation::max);
  }
...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin) {
  ...
  SetUserOutputVariableName(0, "sound_speed");
  SetUserOutputVariableName(1, "dust_ratio_1");
  SetUserOutputVariableName(2, "dust_ratio_2");
  SetUserOutputVariableName(3, "dust_ratio_3");
  SetUserOutputVariableName(4, "dust_ratio_4");
  SetUserOutputVariableName(5, "dust_ratio_5");
      ...
      for (int k=pmb->ks; k<=pmb->ke; k++) {
        for (int j=pmb->js; j<=pmb->je; j++) {
          for (int i=pmb->is; i<=pmb->ie; i++) {
            Real &rad   = pmb->pcoord->x1v(i);
            if ((rad > 1.2) && (rad < 2.4)) {
              Real ratio    = pmb->pdustfluids->df_w(rho_id, k, j, i)/pmb->phydro->w(IDN, k, j, i);
...
#pragma omp simd
        ...
        Real &sound_speed = user_out_var(0, k, j, i);
...
#pragma omp simd
        for(int i=is; i<=ie; i++) {
          ...
          // Dust-Gas Ratio
          Real &ratio = user_out_var(n+1, k, j, i);
